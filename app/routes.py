...
@main.route('/<category_slug>/<subject_slug>/')
def subject(category_slug, subject_slug):
    cat = Category.query.filter_by(slug=category_slug).first_or_404()

    # subject lives in one of cat's subcategories
    subcat_ids = [sc.id for sc in SubCategory.query.filter_by(category_id=cat.id).all()]
    subj = Subject.query.filter(
        Subject.slug == subject_slug,
        Subject.subcategory_id.in_(subcat_ids),
    ).first_or_404()
    ...
    return render_template('subject.html', category=cat, subject=subj, pagination=pagination)


@main.route('/<category_slug>/<subject_slug>/submit/', methods=['POST'])
def submit_review(category_slug, subject_slug):
    ...
    if errors:
        ...
        return render_template('subject.html', category=cat, subject=subj,
...
@main.route('/<category_slug>/<subject_slug>/<int:review_id>/')
def review(category_slug, subject_slug, review_id):
    cat = Category.query.filter_by(slug=category_slug).first_or_404()
    subcat_ids = [sc.id for sc in SubCategory.query.filter_by(category_id=cat.id).all()]
    subj = Subject.query.filter(
        Subject.slug == subject_slug,
    ...
    return render_template('review.html', category=cat, subject=subj, review=rev)
